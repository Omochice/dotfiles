...
export function generate(pathToYabai: string): Array<k.Rule | k.RuleBuilder> {
  const rules: Array<k.Rule | k.RuleBuilder> = [];

  const $$ = (cmd: string, paths: string[] = []): string => {
    const prefix = `export PATH=${["$PATH", ...paths].join(":")};`;
    return `${prefix}${cmd}`;
  };
  const $ = (cmd: string) => $$(cmd, ["$HOME/.deno/bin", pathToYabai]);
