# This module is based on https://github.com/nix-community/home-manager/blob/c47b2cc64a629f8e075de52e4742de688f930dc6/modules/programs/claude-code.nix
{
  config,
  lib,
  pkgs,
...
let
  cfg = config.programs.my-claude-code;
  jsonFormat = pkgs.formats.json { };

  isStorePathString = value: builtins.isString value && lib.types.pathInStore.check value;

  isStorePathFile = value: isStorePathString value && lib.hasSuffix ".md" value;

  isStorePathDirectory = value: isStorePathString value && !(lib.hasSuffix ".md" value);

  isNixPath = builtins.isPath;

  isNixPathDirectory = path: isNixPath path && lib.pathExists path && lib.pathIsDirectory path;
in
{
  options.programs.my-claude-code = {
    enable = lib.mkEnableOption "Enable My claude code program (xdg config based setup)";
    ...
    skills = lib.mkOption {
      ...
      description = ''
        ...
        The attribute name becomes the skill filename or directory name, and the value is one of:
        - Inline content as a string (creates .claude/skills/<name>.md)
        - A Nix path to a file (creates .claude/skills/<name>.md)
        - A Nix path to a directory (creates .claude/skills/<name>/ with all files)
        - A store path string ending with .md, e.g., from nvfetcher (creates .claude/skills/<name>.md)
        - A store path string not ending with .md, e.g., from nvfetcher (creates .claude/skills/<name>/ with all files)
      '';
      example = lib.literalExpression ''
        {
          xlsx = ./skills/xlsx.md;
  ...
  config = lib.mkIf cfg.enable {
    ...
    // lib.mapAttrs' (
      name: content:
      if isStorePathDirectory content then
        lib.nameValuePair "claude/skills/${name}" {
          source = content;
          recursive = true;
        }
      else if isStorePathFile content then
        lib.nameValuePair "claude/skills/${name}/SKILL.md" {
          source = content;
        }
      else if isNixPathDirectory content then
        lib.nameValuePair "claude/skills/${name}" {
          source = content;
          recursive = true;
        }
      else if isNixPath content then
        lib.nameValuePair "claude/skills/${name}/SKILL.md" {
          source = content;
        }
      else
