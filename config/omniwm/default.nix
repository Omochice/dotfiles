...
let
  # OmniWM discards the whole settings file if `hotkeys` omits any action it
  # knows about, so this list has to stay exhaustive;
  # scripts/tools/omniwm-action-ids.ts checks it against the packaged version.
  actionIds = builtins.fromJSON (builtins.readFile ./action-ids.json);

  digits = builtins.genList (i: toString (i + 1)) 9;

  # These ids are zero-based: the "1" key reaches `switchWorkspace.0`.
  workspaceBindings = lib.listToAttrs (
    lib.concatMap (i: [
      (lib.nameValuePair "switchWorkspace.${toString i}" "Command+${toString (i + 1)}")
      (lib.nameValuePair "moveToWorkspace.${toString i}" "Shift+Command+${toString (i + 1)}")
    ]) (lib.range 0 8)
  );
  ...
  # A workspace is created on demand only if its name is configured here.
  workspaces = map (n: {
    ...
    programs.omniwm = {
      ...
      settings = {
        ...
        appRules = [
          ...
          # Arto opens a background helper window titled "tao window"; matching
          # the app itself would float Arto's real windows as well.
          (floatRule "6f1a0d2e-0000-4000-8000-000000000006" {
            bundleId = "";
