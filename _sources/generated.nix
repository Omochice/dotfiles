# This file was generated by nvfetcher, please do not modify it manually.
{
  fetchgit,
  fetchurl,
  fetchFromGitHub,
  dockerTools,
}:
{
  bat-theme-catppuccin = {
    pname = "bat-theme-catppuccin";
    version = "6810349b28055dce54076712fc05fc68da4b8ec0";
    src = fetchFromGitHub {
      owner = "catppuccin";
      repo = "bat";
      rev = "6810349b28055dce54076712fc05fc68da4b8ec0";
      fetchSubmodules = false;
      sha256 = "sha256-lJapSgRVENTrbmpVyn+UQabC9fpV1G1e+CdlJ090uvg=";
    };
    date = "2025-06-29";
  };
  claude-skill-anthropics = {
    pname = "claude-skill-anthropics";
    version = "3b3fad96af16a10759d930941b4520ba0c40edae";
    src = fetchFromGitHub {
      owner = "anthropics";
      repo = "skills";
      rev = "3b3fad96af16a10759d930941b4520ba0c40edae";
      fetchSubmodules = false;
      sha256 = "sha256-nVid8vENmLDh7ffDqh+bJbEWtXcVltA0qa2rItmniZM=";
    };
    date = "2026-08-21";
  };
  claude-skill-ast-grep = {
    pname = "claude-skill-ast-grep";
    version = "6b668aa526afdc623c1a9ed1d6ae920e04a717ad";
    src = fetchFromGitHub {
      owner = "ast-grep";
      repo = "claude-skill";
      rev = "6b668aa526afdc623c1a9ed1d6ae920e04a717ad";
      fetchSubmodules = false;
      sha256 = "sha256-esxl6g4U2+0KPkcDAq0HsIYkZaHM3iYy0rqylM83/bk=";
    };
    date = "2026-08-21";
  };
  claude-skill-lacolaco = {
    pname = "claude-skill-lacolaco";
    version = "442e94de808fa190fb2fa1b86f7edce00c957f8d";
    src = fetchFromGitHub {
      owner = "lacolaco";
      repo = "claude-plugins";
      rev = "442e94de808fa190fb2fa1b86f7edce00c957f8d";
      fetchSubmodules = false;
      sha256 = "sha256-YYBMTJL1ahx41NttMSHZkSva7G1uRAM6cMo93EaYS20=";
    };
    date = "2026-09-06";
  };
  claude-skill-mattpocock = {
    pname = "claude-skill-mattpocock";
    version = "6654f6b60cd9d5be8b54c6fafe44346dabeb3b76";
    src = fetchFromGitHub {
      owner = "mattpocock";
      repo = "skills";
      rev = "6654f6b60cd9d5be8b54c6fafe44346dabeb3b76";
      fetchSubmodules = false;
      sha256 = "sha256-N5tpUIHO2VFeJntBTl6/VLDIVpqoshwFxNJlfXXUwsQ=";
    };
    date = "2026-08-24";
  };
  claude-skill-tani = {
    pname = "claude-skill-tani";
    version = "9d3acf8d309356bbcb1564a7bb50a691ec2bc839";
    src = fetchgit {
      url = "https://git.sr.ht/~tani/skills";
      rev = "9d3acf8d309356bbcb1564a7bb50a691ec2bc839";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-oH7zx2+4JrAyUcno4DYVBDD41oQChIy96miG/FxRhOk=";
    };
    date = "2026-05-16";
  };
  fish-autols = {
    pname = "fish-autols";
    version = "6d704c0e33522335539bf6844ce9f7009b2ee6a2";
    src = fetchFromGitHub {
      owner = "rstacruz";
      repo = "fish-autols";
      rev = "6d704c0e33522335539bf6844ce9f7009b2ee6a2";
      fetchSubmodules = false;
      sha256 = "sha256-tqAsc9J8xv0DMt5fTYaBO7tUQAG7Fnct/Rlq/Jx+/yU=";
    };
    date = "2018-10-09";
  };
  fish-balias = {
    pname = "fish-balias";
    version = "9fb134c097c0aaec15a5f5c79bca8aa6879b1473";
    src = fetchFromGitHub {
      owner = "oh-my-fish";
      repo = "plugin-balias";
      rev = "9fb134c097c0aaec15a5f5c79bca8aa6879b1473";
      fetchSubmodules = false;
      sha256 = "sha256-v9nP3IfsgxH1D6S76lWOaGgelB6IO+z8EZ/2IIYoLR4=";
    };
    date = "2015-11-12";
  };
  fish-catputtin = {
    pname = "fish-catputtin";
    version = "5fc5ae9c2ec22eb376cb03ce76f0d262a38960f3";
    src = fetchFromGitHub {
      owner = "catppuccin";
      repo = "fish";
      rev = "5fc5ae9c2ec22eb376cb03ce76f0d262a38960f3";
      fetchSubmodules = false;
      sha256 = "sha256-3KNWYXfOMzZovdjwjBpjSH8cVlD4CO2QmQcCyQE4Dac=";
    };
    date = "2026-03-13";
  };
  fish-emptyls = {
    pname = "fish-emptyls";
    version = "6b97fc65c7da2a3bcb3ea07075d0fcb5cd444fc1";
    src = fetchFromGitHub {
      owner = "Omochice";
      repo = "emptyls-fish";
      rev = "6b97fc65c7da2a3bcb3ea07075d0fcb5cd444fc1";
      fetchSubmodules = false;
      sha256 = "sha256-czDK4bztyF3tnr1G2yLAf6Ve3HX50rJ4WL7vFj315h0=";
    };
    date = "2021-05-29";
  };
  fish-fzf-emoji = {
    pname = "fish-fzf-emoji";
    version = "7763624950ddf0ae1f1f9559be367f5861183fcc";
    src = fetchFromGitHub {
      owner = "Omochice";
      repo = "fzf-emoji.fish";
      rev = "7763624950ddf0ae1f1f9559be367f5861183fcc";
      fetchSubmodules = false;
      sha256 = "sha256-H4Mk2T6RnrxWcG4mKUdaGk/Te0rbbql4Go8uo+aP8UI=";
    };
    date = "2022-12-28";
  };
  gh-fzgist = {
    pname = "gh-fzgist";
    version = "e2d20a91bbd19de1dc1cd8c603b366dd63c92158";
    src = fetchFromGitHub {
      owner = "Omochice";
      repo = "gh-fzgist";
      rev = "e2d20a91bbd19de1dc1cd8c603b366dd63c92158";
      fetchSubmodules = false;
      sha256 = "sha256-Cweij9Y1qfoqBI8UumTzbMbXDjeqJc3ytISFir7g4Vg=";
    };
    date = "2021-12-11";
  };
  gh-q = {
    pname = "gh-q";
    version = "d2d2e466c63a074fdf7e731a1f0d807d239f0a6b";
    src = fetchFromGitHub {
      owner = "kawarimidoll";
      repo = "gh-q";
      rev = "d2d2e466c63a074fdf7e731a1f0d807d239f0a6b";
      fetchSubmodules = false;
      sha256 = "sha256-jRg8Pbs45UmIuejnrrBxlqvs9p3yN4LO37dK0Mo2ACA=";
    };
    date = "2025-12-30";
  };
  gh-userfetch = {
    pname = "gh-userfetch";
    version = "9c90b188f1c58a6a3d6ce5f9220b05b5414b040f";
    src = fetchFromGitHub {
      owner = "sheepla";
      repo = "gh-userfetch";
      rev = "9c90b188f1c58a6a3d6ce5f9220b05b5414b040f";
      fetchSubmodules = false;
      sha256 = "sha256-lLmKUphsH8ByQPqsGrIXYgLlJQntmcqvLDxCnfiXvmk=";
    };
    date = "2021-09-19";
  };
  neovim = {
    pname = "neovim";
    version = "53074f2978092d22da341793b5f2a68247d8898e";
    src = fetchFromGitHub {
      owner = "neovim";
      repo = "neovim";
      rev = "53074f2978092d22da341793b5f2a68247d8898e";
      fetchSubmodules = false;
      sha256 = "sha256-6IHvCRNGSzxDTAQdRQ37kW8X+p4OH0eiCsHqD2D3V1g=";
    };
    date = "2026-09-18";
  };
  tree-sitter-moonbit = {
    pname = "tree-sitter-moonbit";
    version = "5435c307c6cf2ef0d508a99047b06f35a4308444";
    src = fetchFromGitHub {
      owner = "moonbitlang";
      repo = "tree-sitter-moonbit";
      rev = "5435c307c6cf2ef0d508a99047b06f35a4308444";
      fetchSubmodules = false;
      sha256 = "sha256-UUEjrF6uGwTtFGRjmjw75ky8eDwVwAHOHro48TAI+WM=";
    };
    date = "2026-07-22";
  };
  yazi-theme-catppuccin = {
    pname = "yazi-theme-catppuccin";
    version = "d62802be39210ea10e54b3e3b09735c6cb9e57c1";
    src = fetchFromGitHub {
      owner = "catppuccin";
      repo = "yazi";
      rev = "d62802be39210ea10e54b3e3b09735c6cb9e57c1";
      fetchSubmodules = false;
      sha256 = "sha256-bwzEO8exoBwa19q+jnYjHkaamGl2mhfukIEhDfUCRGI=";
    };
    date = "2026-08-20";
  };
  zathura-catputtin = {
    pname = "zathura-catputtin";
    version = "9f29c2c1622c70436f0e0b98fea9735863596c1e";
    src = fetchFromGitHub {
      owner = "catppuccin";
      repo = "zathura";
      rev = "9f29c2c1622c70436f0e0b98fea9735863596c1e";
      fetchSubmodules = false;
      sha256 = "sha256-upyfc4OSx9xKUoM/JdRfuXiw38ffoSB/Utm2jpyXgy8=";
    };
    date = "2025-08-03";
  };
}
